...
commonAnimals <- intersect(x = mammals, y = Animals2)
...
# Animales en Animals2 que no estén mammals
diferentes1<-setdiff(Animals2, mammals)
rownames(diferentes1)

# Hay 3 animales diferentes
# Esos son: Diplodocus, Triceratops y Brachiosaurus

# Animales en mammals que no estén en Animals2
diferentes2<- setdiff(mammals, Animals2)
rownames(diferentes2)
...
# No existe una correspondencia entre el peso de sus cuerpos y el de sus cerebros. También se diferencian en el peso tanto de cuerpo como de cerebro.
...
# Observamos que presentan el mismo comportamiento

# Vemos que los puntos están dentro de las líneas, por tanto podemos afirmar que siguen una distribución Gaussiana
...
# Tengo instaladas 282 librerías
...
# Apartado A
# Determinamos el número de variables y registros
str(cabbages)

# Tiene 60 registros de 4 variables, tres de las variables son Factors y la otra es numérica

# Apartado B
colSums(is.na(x = cabbages))

# Apartado C
str(Chile)
colSums(is.na(x = Chile))
...
# Podemos observar que ambas variables no sales de las líneas que definen a la distribución Gaussiana, por tanto verifican la condición
...
# Podemos ver como para c52 los valores son mucho más altos mientras que para c39 se quedan "estancados" en 55
...
ModelA <- lm(VitC ~ HeadWt, data = cabbages)
ModelB <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c39"))
ModelC <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c52"))
abline(ModelA, lty = 1, col = "green")
abline(ModelB, lty = 2)
abline(ModelC)
